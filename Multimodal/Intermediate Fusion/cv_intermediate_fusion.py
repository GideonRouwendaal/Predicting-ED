...
def objective(trial, fold_idx, args, cf):
    ...
    cf["normalize_override"] = (None, None)
    pat = re.compile(r'([Aa]?nonymized Patient \d+)(?:_slice_\d+)?\.nrrd$')
    for inner_idx in range(args.n_inner_folds):

        cf["data_path"] = <path to images of multiple 2D slices>
        ...
        cf["data_path"] = <path to single slice MRI images> 
        _, val_ds = prepare_fold_dataset(args.folds_path, fold_idx, 'inner', inner_idx, args=SimpleNamespace(**cf))
...
def outer_cv(args):
    ...
    for fold_idx in tqdm(range(0, args.n_outer_folds)):
        ...
        study.optimize(lambda trial: objective(trial, fold_idx, args, temp_cf), n_trials=args.n_trials)
        ...
        best_metrics, trues, preds = train_and_evaluate(fusion_model, train_loader, val_loader, args, cf, cf, cf, preprocessor, train_df, val_df, class_weights=weights)
        ...
        save_dir = f"Final/Models/Multimodal/intermediate_fusion/fold_{fold_idx}"
        ...
        np.savez(preds_path, preds=preds, trues=trues)
