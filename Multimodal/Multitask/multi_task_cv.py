import argparse
from Dataset.dataset_2D_multi_task import prepare_fold_dataset
...
def objective(trial, fold_idx, args, weights):
    ...
    for inner_idx in range(args.n_inner_folds):
        ...
        train_ds, val_ds = prepare_fold_dataset(
            args.folds_path, fold_idx, 'inner', inner_idx, args=SimpleNamespace(**cf), age_dict=age_dict 
        )
...
def outer_cv(args):
    ...
    for fold_idx in tqdm(range(0, args.n_outer_folds)):
        ...
        temp_train_ds, _ = prepare_fold_dataset(args.folds_path, fold_idx, 'outer', args=SimpleNamespace(**temp_cf), age_dict=age_dict)
        weights = compute_class_weights_from_dataset(temp_train_ds)

        print(f"Class weights: {weights}")

        study = optuna.create_study(direction="maximize")
        study.optimize(lambda trial: objective(trial, fold_idx, args, weights), n_trials=args.n_trials)
        ...
        if "multiple_2D" in args.data_path:
            save_dir = f"Final/Models/Multi_Task/multiple_2D_slices/{cf['model_type']}/{pretrained}/fold_{fold_idx}"
        else:
            save_dir = f"Final/Models/Multi_Task/{cf['model_type']}/{pretrained}/fold_{fold_idx}"
