...
def prepare_df(fascia_thickness_dict, df, to_predict_label):
    ...
    selected_patients = list(fascia_thickness_dict.keys())
...
def get_region_stat_dict_all(slice_region_values, stat='mean'):
    regions = ['R1', 'R2', 'R3', 'R4', 'R5', 'R6', 'L6', 'L5', 'L4', 'L3', 'L2', 'L1']
    region_values = {region: [] for region in regions}

    for patient_data in slice_region_values.values():
        for slice_key, value in patient_data.items():
            region = '_'.join(slice_key.split('_')[1:]).split('_')[-1]
            if region in region_values:
                region_values[region].append(value)

    if stat == 'mean':
        return {region: np.mean(values) if values else 0 for region, values in region_values.items()}
    elif stat == 'median':
        return {region: np.median(values) if values else 0 for region, values in region_values.items()}

    raise ValueError("Unsupported stat type. Choose 'mean' or 'median'.")


def plot_overall_dodecagons(slice_region_values, output_dir, largest_length, stat='mean'):
    os.makedirs(output_dir, exist_ok=True)

    # Individual Patient Plots
    # for patient, data in slice_region_values.items():
    #     stat_dict = get_region_stat_dict_all({patient: data}, stat=stat)
    #     img_output_path = os.path.join(output_dir, f"{patient}_{stat}.png")
    #     plot_dodecagon(img_output_path, stat_dict, largest_length, patient=f"{patient} - {stat}")

    # Aggregate Plot
    overall_stat_dict = get_region_stat_dict_all(slice_region_values, stat=stat)
    overall_img_path = os.path.join(output_dir, f"overall_{stat}.png")
    plot_dodecagon(overall_img_path, overall_stat_dict, largest_length, patient=f"Overall - {stat}")
...
def find_best_model(subset_df, features, subset_df_test, features_test, folds):
    ...
    for fold_name, fold_data in tqdm(folds.items(), desc="Outer Fold Progress"):
        ...
        for model_name, pipeline in possible_models.items():
            ...
            save_dir = os.path.join(f"Final/Models/FT_multiple/{model_name}", fold_name)
...
def get_feature_importances(best_models, features):
    ...
    for name, model in best_models.items():
        clf = model.named_steps["clf"]  # Access classifier inside the pipeline
        ...
        if hasattr(clf, "coef_"):  # LogisticRegression
            ...
            print(f"Skipping feature importance for {name} (not supported).")
            continue

        model_feature_importances[name] = dict(zip(features, importances))
...
def plot_importances(aggregated_importances, title, output_path):
...
img_output_path = <output path of generated imgs>
...
for approach in approaches:
    ft_path = <path to ft dictionary of all slices>
    ...
    flattened_fascia_thickness_dict = flatten_fascia_thickness_dict(fascia_thickness_dict)
    ...
    if type(to_predict_label) == list:
        for label in to_predict_label:
            if to_plot:
                ...
                print(os.path.join(dodecagon_output_path, "overall_ed.png"))
                print(os.path.join(dodecagon_output_path, "overall_no_ed.png"))
            ...
            for model_name, importances in feature_importances.items():
                slice_agg = aggregate_importances(importances, mode="slice")
                region_agg = aggregate_importances(importances, mode="region")
                plot_importances(slice_agg,
                                title=f"{model_name} - Feature Importance per Slice",
                                output_path=os.path.join(importance_dir, f"slice_{model_name}.png"))

                plot_importances(region_agg,
