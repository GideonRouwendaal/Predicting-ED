...
def process_patient(patient, full_img_path, full_segmentation_path, data_path_full):
    ...
    middle_img, fascia_segment = sitk.GetArrayFromImage(resampled_img)[mid_prostate_slice_idx, :, :], sitk.GetArrayFromImage(resampled_seg)[mid_prostate_slice_idx, :, :]

    height, width = middle_img.shape

    prostate_mask = fascia_segment == prostate_label
    fascia_mask = (fascia_segment == fascia_label).astype(int)
    ...
    if prostate_center != False:
        ...
        median_lengths, _ = calculate_median_lengths(lines_sub_angles, spacing)
        rearranged_dict = rearrange_dictionary(median_lengths)
        
        return {i: value for i, (_, value) in enumerate(rearranged_dict.items())}
    else:
        print(f"For {patient}, no prostate center can be found" )
        return False
...
with open(os.path.join(dict_output_path, "fascia_thickness_single_slice_mid_prostate.json"), "w") as f:
