import argparse
from Dataset.dataset_2D import prepare_fold_dataset
...
def outer_cv(args):
    ...
    for fold_idx in tqdm(range(args.n_outer_folds)):
        ...
        temp_train_ds, _ = prepare_fold_dataset(args.folds_path, fold_idx, 'outer', args=SimpleNamespace(**temp_cf))
        ...
        if "multiple_2D" in args.data_path:
            save_dir = f"Final/Models/multiple_2D_slices/{cf['model_type']}/{pretrained}/fold_{fold_idx}"
        else:
            save_dir = f"Final/Models/single_slice/{cf['model_type']}/{pretrained}/fold_{fold_idx}"
