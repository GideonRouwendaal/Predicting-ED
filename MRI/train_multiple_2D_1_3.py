...
def build_config(args, trial=None, fixed_params=None):
    ...
    if trial:
        ...
        if args.model_type == "hybrid_rvit":
            ...
            cf["img_size"] = 512
            cf["patch_size"] = int(fixed_params["patch_size"])
...
def train_and_evaluate(model, train_loader, val_loader, cf, class_weights=None):
    ...
    all_preds, all_trues, all_outputs_list = [], [], []
    ...
    return best_metrics, all_trues, all_preds, all_outputs_list
...
def outer_cv(args):
    ...
    for fold_idx in tqdm(range(args.n_outer_folds)):
        ...
        best_params_file = f"Final/Models/{args.model_type}/{pretrained}/fold_{fold_idx}/metrics_and_params.txt"
        best_params = load_best_params(best_params_file)
        cf = build_config(args, fixed_params=best_params)
        print(cf)
        cf["data_path"] = <path to multiple 2D slices>
        ...
        model = build_model(SimpleNamespace(**cf))
        cf["data_path"] = <path to single slice 2D slices>
        ...
        save_dir = f"Final/Models/multiple_2D_slices_1_3/{cf['model_type']}/{pretrained}/fold_{fold_idx}"
