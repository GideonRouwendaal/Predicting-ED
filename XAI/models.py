...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x):
        x_attn, attn_weights = self.attn(self.norm1(x), self.norm1(x), self.norm1(x), need_weights=True, average_attn_weights=True)
...
class ResNetViT(nn.Module):
    ...
    def forward(self, x):
        ...
        for layer in self.transformers:
            x = layer(x)
            cls_attention = layer.attn_weights[:, 0, 1:]  # Get attention weights for class token
            attn_weights.append(cls_attention)
        self.attention_maps = torch.stack(attn_weights, dim=1)
