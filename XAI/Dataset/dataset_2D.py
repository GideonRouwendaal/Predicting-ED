...
class FolderDatasetResNet(Dataset):
    def __init__(self, folder, allowed_patient_ids=None, undersample=False, augment=True):
        self.folder = folder
        ...
        self.image_paths = ed_images + no_ed_images
...
class FolderDatasetViT(Dataset):
    def __init__(self, folder, allowed_patient_ids=None, undersample=False, augment=True, normalize_images=True, model_type='vit'):
...
def prepare_fold_dataset_multimodal(folds_path, fold_idx, split_level='outer', inner_idx=None, args=None):
    ...
    if args.model_type in ['vit', 'hybrid_rvit']:
        temp_train_ds = generate_dataset(args, allowed_patient_ids=train_ids, augment=False)
    ...
    train_ds = generate_dataset(args, allowed_patient_ids=train_ids, augment=False)
    val_ds   = generate_dataset(args, allowed_patient_ids=val_ids, augment=False)
...
def generate_dataset(args, allowed_patient_ids=None, augment=False):
    mean_override, std_override = getattr(args, "normalize_override", (None, None))

    if args.model_type == 'resnet':
        dataset = FolderDatasetResNet(args.data_path, augment=augment, allowed_patient_ids=allowed_patient_ids)

    elif args.model_type in ['vit', 'hybrid_rvit']:
        dataset = FolderDatasetViT(args.data_path, augment=augment, model_type=args.model_type, normalize_images=False, allowed_patient_ids=allowed_patient_ids)
...
def prepare_fold_dataset(folds_path, fold_idx, split_level='outer', inner_idx=None, args=None):
    with open(folds_path, 'r') as f:
        folds = json.load(f)
    ...
    train_ds = generate_dataset(args, allowed_patient_ids=train_ids, augment=True)
    val_ds   = generate_dataset(args, allowed_patient_ids=val_ids, augment=False)
