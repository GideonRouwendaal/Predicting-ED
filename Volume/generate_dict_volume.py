...
def compute_volumes_from_segmentations(segmentation_array, spacing, prostate_label=1, fascia_label=2):
    dx, dy, dz = spacing
    voxel_volume = dx * dy * dz  # in mm³

    prostate_volume_mm3 = np.sum(segmentation_array == prostate_label) * voxel_volume
    fascia_volume_mm3 = np.sum(segmentation_array == fascia_label) * voxel_volume

    return prostate_volume_mm3, fascia_volume_mm3
...
def load_img_and_segmentation(full_img_path, full_segmentation_path, patient):
    try:
        (img, _), (segmentation, seg_header) = nrrd.read(full_img_path), nrrd.read(full_segmentation_path)
...
def process_patient(patient, full_img_path, full_segmentation_path, data_path_full):
    ...
    prostate_volume, fascia_volume = compute_volumes_from_segmentations(
        prostate_fascia_segmentation[prostate_indices],
        spacing,
        prostate_label,
        fascia_label
    )
    result_dict['prostate_volume_mm3'] = prostate_volume
    result_dict['fascia_volume_mm3'] = fascia_volume
...
for file in tqdm(mapping):
    ...
    volume_dict = process_patient(patient, full_img_path, full_segmentation_path, data_path_full)
    if not volume_dict:
        no_middle_slice.append(patient)
    else:
        prostate_volume_mm3 = volume_dict.pop('prostate_volume_mm3')
        fascia_volume_mm3 = volume_dict.pop('fascia_volume_mm3')
        volume_dict[patient] = {
            'prostate_volume_mm3': prostate_volume_mm3,
            'fascia_volume_mm3': fascia_volume_mm3,
            'prostate_volume_ml': prostate_volume_mm3 / 1000,
            'fascia_volume_ml': fascia_volume_mm3 / 1000
        }

with open(os.path.join(dict_output_path, "volume_dict.json"), "w") as f:
